...
function ProjectCard({
  ...
  return (
    ...
    >
      <div
        className={`ml-auto mr-auto relative items-center md:flex  ${inverse ? "md:flex-row-reverse" : "md:flex-row"}`}
      >
        <div
          className={`p-2 sm:p-5 md:p-0 relative  z-30 md:w-1/2 w-full flex flex-col justify-around
        ...
        >
          <div className="ml-5 mr-5 mb-5">
            <a
              ...
              className="hover:text-mygreen duration-500 font-bold text-2xl text-lightest-slate"
            >
              {title}
            </a>
          </div>
          ...
          >
            {tecnologies.map((item) => {
              return (
                <p
                  className={`${tecnologies.indexOf(item) !== 0 ? "sm:ml-3" : ""} text-mydarkgreen hover:text-mygreen hover:scale-125 duration-500 cursor-default`}
                  key={`${title}-${item}`}
                >
                  {item}
                </p>
        ...
        <div
          className={`md:shadow-c1 hover:shadow-c2 duration-300 absolute top-0  w-full h-full md:relative z-10 md:w-1/2 hover:cursor-pointer
